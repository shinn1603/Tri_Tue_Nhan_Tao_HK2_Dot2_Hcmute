...
class AnimatePuzzleGUI:
    ...
    def setup_ui(self):
        ...
        tk.Label(self.left_frame, text="Mô phỏng Trực quan", font=("Helvetica", 14, "bold")).pack(pady=5)
        ...
        self.btn_gen = tk.Button(top_ctrl_frame, text="Tạo Puzzle Mới", command=self.generate_new_puzzle, font=("Helvetica", 10))
        self.btn_gen.pack(side=tk.LEFT, padx=5)
        
        self.btn_solve = tk.Button(top_ctrl_frame, text="Giải (BFS)", command=self.start_solve, bg="#4CAF50", fg="white", font=("Helvetica", 10, "bold"))
        ...
        self.speed_scale = tk.Scale(speed_frame, from_=100, to=5000, resolution=100, orient=tk.HORIZONTAL, length=180)
        self.speed_scale.set(1500)
    ...
    def generate_new_puzzle(self):
        ...
        self.status_lbl.config(text="Đã tạo puzzle mới. Sẵn sàng giải", fg="black")
    ...
    def start_solve(self):
        ...
        self.disable_anim_controls()
        self.stop_current_animation()
        self.status_lbl.config(text="Đang tìm kiếm đường đi bằng BFS...", fg="red")
        self.history_listbox.delete(0, tk.END)
    ...
    def on_solve_complete(self, solution_node, steps_searched, duration):
        ...
        if solution_node:
            ...
            self.status_lbl.config(text=f" BFS xong ({duration:.2f}s). Duyệt {steps_searched} trạng thái.\nGiải thành công: {len(self.path)-1} bước", fg="#4CAF50")
    ...
    def populate_history(self):
        ...
        for i, node in enumerate(self.path):
            if node.move_action is None:
                self.history_listbox.insert(tk.END, f"[0] Bắt đầu")
            else:
                self.history_listbox.insert(tk.END, f"[{i}] Di chuyển ô vào ô trống: {node.move_action}")
