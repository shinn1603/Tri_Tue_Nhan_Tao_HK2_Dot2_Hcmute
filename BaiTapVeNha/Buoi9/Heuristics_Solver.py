...
def is_solvable(start, target):
    # Đối với puzzle 3x3, trạng thái giải được khi tính chẵn lẻ của số nghịch thế bằng nhau
...
class PuzzleGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("8-Puzzle Solver Heuristic")
    ...
    def setup_ui(self):
        ...
        matrix_frame = tk.LabelFrame(self.right_frame, text="Dữ liệu đầu vào", padx=5, pady=5)
        ...
        self.algo_combo['values'] = ("Greedy", "A*", "Leo đồi", "Leo đồi dốc nhất")
    ...
    def apply_input(self):
        try:
            ...
            if not is_solvable(new_start, new_target):
    ...
    def start_solve(self):
        ...
        if algo == "Greedy":
            threading.Thread(target=self.greedy_worker, daemon=True).start()
        elif algo == "A*":
            threading.Thread(target=self.astar_worker, daemon=True).start()
        elif algo == "Leo đồi":
            threading.Thread(target=self.hill_climbing_worker, daemon=True).start()
        else:
            threading.Thread(target=self.steepest_ascent_worker, daemon=True).start()

    def process_solution(self, solution_node):
        self.is_searching = False
        ...
        if not solution_node:
            self.log_trace_sync("-> Thuật toán bế tắc (Không thể tìm được đường đi)")
            messagebox.showinfo("Thông báo", "Thuật toán bế tắc hoặc cạn kiệt trạng thái.")
            return
    ...
    def greedy_worker(self):
        ...
        while not queue.empty() and self.is_searching:
            ...
            if show_log:
                self.log_trace_sync(f"[Bước {self.stats['nodes']}] Lấy khỏi hàng đợi (h={current_h}): {cur_node.state.flatten()}")

            if np.array_equal(cur_node.state, self.target_puzzle):
                self.root.after(0, self.process_solution, cur_node)
                return
            ...
            for move in get_move(cur_node.row, cur_node.col):
                ...
                if state_tuple not in explored:
                    ...
                    push_count += 1
                    added += 1
                    if show_log: self.log_trace_sync(f"  + Thêm hướng {move} (h={new_h})")
                else:
                    skipped += 1
        ...
        self.root.after(0, self.process_solution, None)
    ...
    def astar_worker(self):
        ...
        while not queue.empty() and self.is_searching:
            ...
            show_log = self.stats["nodes"] <= 30
            if show_log:
                self.log_trace_sync(f"[Bước {self.stats['nodes']}] Lấy khỏi hàng đợi (f={current_f}, g={current_g}): {cur_node.state.flatten()}")
            ...
            for move in get_move(cur_node.row, cur_node.col):
                ...
                if state_tuple not in explored:
                    ...
                    push_count += 1
                    added += 1
                    if show_log: self.log_trace_sync(f"  + Thêm hướng {move} (f={new_f}, g={new_g}, h={new_h})")
                else:
                    skipped += 1
                    if show_log: self.log_trace_sync(f"  - Bỏ qua hướng {move} (trùng lặp)")
    ...
    def hill_climbing_worker(self):
        ...
        while self.is_searching:
            ...
            show_log = self.stats["nodes"] <= 30
            if show_log:
                self.log_trace_sync(f"[Bước {self.stats['nodes']}] Xét đỉnh (h={current_h}): {current_node.state.flatten()}")

            if np.array_equal(current_node.state, self.target_puzzle):
                self.root.after(0, self.process_solution, current_node)
                return
            ...
            for move in get_move(current_node.row, current_node.col):
                ...
                if state_tuple not in explored:
                    new_h = pathcost(new_puzzle, self.target_puzzle)
                    if new_h < current_h:
                        if show_log: self.log_trace_sync(f"  + Chọn luôn hướng {move} (h={new_h} < {current_h})")
                        explored.add(state_tuple)
                        current_node = NODE(new_puzzle, current_node, move, new_r, new_c)
                        found_better = True
                        break 
                    else:
                        if show_log: self.log_trace_sync(f"  - Bỏ qua hướng {move} (h={new_h} >= {current_h})")
                else:
                    if show_log: self.log_trace_sync(f"  - Bỏ qua hướng {move} (đã duyệt)")

            if not found_better:
                self.log_trace_sync(f"-> Thuật toán bế tắc. Không có đỉnh kề nào tốt hơn {current_h}.")
                self.root.after(0, self.process_solution, None)
                return
                
            if show_log: self.log_trace_sync("")
    ...
    def steepest_ascent_worker(self):
        ...
        while self.is_searching:
            ...
            for move in get_move(current_node.row, current_node.col):
                ...
                if state_tuple not in explored:
                    new_h = pathcost(new_puzzle, self.target_puzzle)
                    if show_log: self.log_trace_sync(f"  * Xét hướng {move} (h={new_h})")
                    if new_h < best_h:
                        best_h = new_h
                        best_move = move
            ...
            if best_node is not None:
                if show_log: self.log_trace_sync(f"  + Chọn hướng tốt nhất {best_move} (h={best_h} < {current_h})\n")
