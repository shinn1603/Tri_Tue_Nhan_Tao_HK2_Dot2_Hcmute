...
class PuzzleGUI:
    ...
    def setup_ui(self):
        ...
        self.algo_combo['values'] = ("BFS", "DFS", "IDS", "UCS", "Greedy", "A*")
    ...
    def process_solution(self, solution_node, algo_name):
        ...
        self.log_trace_sync("Quá trình duyệt kết thúc.")
                ...
                if cur_depth < depth_limit:
                    ...
                    for move in valid_moves:
                        ...
                        if state_tuple not in explored or explored[state_tuple] > cur_depth + 1:
                            ...
                            skipped += 1
                            if show_log: self.log_trace_sync(f"  - Bỏ qua hướng {move}")
                else:
                    if show_log: self.log_trace_sync(f"  -> Chạm giới hạn {depth_limit}, dừng nhánh.")
    ...
    def ucs_worker(self):
        ...
        while not queue.empty() and self.is_searching:
            ...
            show_log = self.stats["nodes"] <= 30
            if show_log:
                self.log_trace_sync(f"[Bước {self.stats['nodes']}] Lấy khỏi hàng đợi (cost={g_cost}): {cur_node.state.flatten()}")
    ...
    def astar_worker(self):
        ...
        # Tuple lưu trữ: (f_cost, push_count, g_cost, node)
