...
class PuzzleGUI:
    ...
    def apply_input(self):
        try:
            ...
            if not check_solvable(new_start, new_target):
                messagebox.showerror("Lỗi", "Trạng thái bắt đầu và đích không cùng tính chẵn lẻ.")
                return
    ...
    def start_solve(self):
        ...
        elif algo == "IDA*": threading.Thread(target=self.ida_star_worker, daemon=True).start()
        ...
        elif algo == "Luyện kim": threading.Thread(target=self.simulated_annealing_worker, daemon=True).start()
    ...
    def process_solution(self, solution_node, algo_name):
        ...
        if not solution_node:
            self.log_trace_sync(f"Thuật toán dừng.")
    ...
    def astar_worker(self):
        ...
        while not queue.empty() and self.is_searching:
            ...
            show_log = self.stats["nodes"] <= 30
            if show_log: self.log_trace_sync(f"[Bước {self.stats['nodes']}] Trích xuất PriorityQueue (f={current_f})")
    ...
    def ida_star_worker(self):
        row, col = Find_blank(self.puzzle)
        start_node = NODE(self.puzzle, None, None, row, col)
        threshold = pathcost(start_node.state, self.target_puzzle)

        def search(path, g, bound):
            if not self.is_searching: return None, float('inf')
            node = path[-1]
            self.stats["nodes"] += 1
            if self.stats["nodes"] % 100 == 0:
                self.stats["memory"] = len(path)
                
            h = pathcost(node.state, self.target_puzzle)
            f = g + h
            
            show_log = self.stats["nodes"] <= 30
            if show_log: self.log_trace_sync(f"[Bước {self.stats['nodes']}] f={f}, bound={bound}")
                
            if f > bound: return None, f
            if np.array_equal(node.state, self.target_puzzle): return node, f
                
            min_bound = float('inf')
            for move in get_move(node.row, node.col):
                new_puzzle, new_r, new_c = action(node.state, move, node.row, node.col)
                state_tuple = tuple(new_puzzle.flatten())
                if state_tuple in [tuple(n.state.flatten()) for n in path]: continue
                    
                child = NODE(new_puzzle, node, move, new_r, new_c)
                path.append(child)
                result, child_f = search(path, g + 1, bound)
                path.pop()
                
                if result is not None: return result, child_f
                if child_f < min_bound: min_bound = child_f
            return None, min_bound

        while self.is_searching:
            self.log_trace_sync(f"\n--- Giới hạn f = {threshold} ---")
            solution, new_threshold = search([start_node], 0, threshold)
            if solution is not None:
                self.root.after(0, self.process_solution, solution, "IDA*")
                return
            if new_threshold == float('inf'): break
            threshold = new_threshold
        self.root.after(0, self.process_solution, None, "IDA*")
    ...
    def hill_climbing_worker(self):
        ...
        while self.is_searching:
            self.stats["nodes"] += 1
            if self.stats["nodes"] % 10 == 0: self.stats["memory"] = len(explored)
            ...
            for move in get_move(current_node.row, current_node.col):
                ...
                if state_tuple not in explored:
                    ...
                    if new_h < current_h:
                        ...
                        found_better = True
                        if show_log: self.log_trace_sync(f"  + Hướng {move} (h={new_h} < {current_h})\n")
                        break 
            if not found_better:
                self.log_trace_sync(f"-> Cực đại cục bộ.")
    ...
    def steepest_ascent_worker(self):
        ...
        while self.is_searching:
            ...
            if best_node is not None:
                ...
                if show_log: self.log_trace_sync(f"  + Chọn hướng {best_move} (h={best_h})\n")
            else:
                self.log_trace_sync(f"-> Cực đại cục bộ.")
    ...
    def random_restart_worker(self):
        ...
        while self.is_searching:
            ...
            if best_node is not None:
                explored.add(tuple(best_node.state.flatten()))
                current_node = best_node
                if show_log: self.log_trace_sync(f"  + Hướng {best_move} (h={best_h})\n")
            else:
                if restarts < max_restarts:
                    restarts += 1
                    self.log_trace_sync(f"-> Cực đại cục bộ. Khởi động vòng lặp (Lần {restarts})")
                    for _ in range(5):
                    ...
                    self.log_trace_sync(f"-> Vượt quá giới hạn lặp.")
    ...
    def local_beam_search_worker(self):
        ...
        while beam and self.is_searching:
            ...
            show_log = self.stats["nodes"] <= 30
            if show_log: self.log_trace_sync(f"[Bước {self.stats['nodes']}] Trạng thái duy trì: {len(beam)}")
            ...
            if not next_candidates:
                self.log_trace_sync(f"-> Trạng thái cạn kiệt.")
            ...
            if show_log:
                ...
                self.log_trace_sync(f"  + Lọc k trạng thái (h tối thiểu = {best_h})\n")
            elif self.stats["nodes"] == 31:
                self.log_trace_sync("... (Ẩn log chi tiết tiếp theo) ...")

    def simulated_annealing_worker(self):
        row, col = Find_blank(self.puzzle)
        current_node = NODE(self.puzzle, None, None, row, col)
        ...
        while T > T_min and self.is_searching:
            self.stats["nodes"] += 1
            if self.stats["nodes"] % 10 == 0: self.stats["memory"] = 1
                
            current_h = pathcost(current_node.state, self.target_puzzle)
            show_log = self.stats["nodes"] <= 30
            
            if show_log: self.log_trace_sync(f"[Bước {self.stats['nodes']}] T={T:.3f}, h={current_h}")
                
            if np.array_equal(current_node.state, self.target_puzzle):
                self.root.after(0, self.process_solution, current_node, "Luyện kim")
                return
                
            valid_moves = get_move(current_node.row, current_node.col)
            move = random.choice(valid_moves)
            new_puzzle, new_r, new_c = action(current_node.state, move, current_node.row, current_node.col)
            next_h = pathcost(new_puzzle, self.target_puzzle)
            
            delta = next_h - current_h
            
            if delta < 0:
                current_node = NODE(new_puzzle, current_node, move, new_r, new_c)
                if show_log: self.log_trace_sync(f"  + Delta < 0. Chấp nhận.")
            else:
                p = np.exp(-delta / T)
                if random.uniform(0, 1) < p:
                    current_node = NODE(new_puzzle, current_node, move, new_r, new_c)
                    if show_log: self.log_trace_sync(f"  + p={p:.3f}. Chấp nhận.")
                else:
                    if show_log: self.log_trace_sync(f"  - p={p:.3f}. Từ chối.")
                    
            T = alpha * T
            
        self.root.after(0, self.process_solution, None, "Luyện kim")
